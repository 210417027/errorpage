...
const ChoosePage = () => {
    ...
    return (
        ...
        <div class="buttons">
            ...
            <Link to={`/errorpage?inputValue=${encodeURIComponent(inputValue)}`} className="submit"><button class="btn-hover color-2">直接點餐</button></Link>
            {/* <Link to={`https://da-xiong-studio-business-card.vercel.app/random/index.html`}><button class="btn-hover color-1">隨機抽選</button></Link> */}
            <Link to={`/errorpage?inputValue=${encodeURIComponent(inputValue)}`} className="submit"><button class="btn-hover color-1">隨機抽選</button></Link>
